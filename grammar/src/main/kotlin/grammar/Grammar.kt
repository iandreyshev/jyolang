...
class Grammar internal constructor(rulesList: List<Rule>) {
    ...
    val nonTerminals: Set<NonTerminal> = rules.nonTerminals.toSet()
    val terminals: Set<Terminal> = rules.terminals.toSet()
    ...
    private fun getFirstSetMap(rules: GrammarRules): Map<GrammarSymbol, Set<Terminal>> {
    ...
    private fun first(symbol: GrammarSymbol, rules: GrammarRules): Set<Terminal> {
